...
getfilterDoer.get('/getfilterDoer', async (req, res) => {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    ...
    errorLogger.log("error", `Token ${req.headers.authorization} is un-authorized as authorization header missing or malformed for api getfilterDoer`);
    return res.status(401).json({ error: "Unauthorized" });
  }
  const token = authHeader.split(" ")[1];
  infoLogger.log("info", `token ${token} is verified successfuly for the api getfilterDoer`);
  ...
  try {
    ...
    infoLogger.log("info", `${userName} from company ${companyUrl} hit the api getfilterDoer`)
